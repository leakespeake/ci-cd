...
# USES THE 'TERRAFORM_REMOTE_STATE' DATA SOURCE TO READ DATA FROM THE STATE FILE OF AN AWS RDS DATABASE - PULLING DB ADDRESS AND PORT INFO
# ALSO WE EXTERNALIZE THE BASH SCRIPT USED WITH 'USER_DATA' (user-data.sh) AND LOAD ITS CONTENTS WITH THE 'FILE' FUNCTION - TO DISPLAY DB ADDRESS AND PORT INFO
...
resource "aws_launch_configuration" "example-config" {
    ...
    user_data       = data.template_file.user_data.rendered             # Read from the 'template_file' DATA SOURCE at line 44

# Pass a shell script to User Data to write text into index.html then use busybox to launch a webserver on 8080 to serve it
...
#                echo "${data.terraform_remote_state.db.outputs.address}" >> index.html  # Reads from DATA SOURCE at line 185
#                echo "${data.terraform_remote_state.db.outputs.port}" >> index.html     # Reads from DATA SOURCE at line 185
...
# Rather than defining the Bash script inline (line 29) - lets externalize it by creating user-data.sh and loading its contents with the 'file' FUNCTION
data "template_file" "user_data" {
    template = file("user-data.sh")

    vars = {
        server_port = var.server_port
        db_address  = data.terraform_remote_state.db.outputs.address        # Reads from DATA SOURCE at line 185
        db_port     = data.terraform_remote_state.db.outputs.port           # Reads from DATA SOURCE at line 185
    }
}
...
# Add a 'terraform_remote_state' DATA SOURCE to allow us to READ the MySQL database state file - via an ATTRIBUTE REFERENCE of format;
# data.terraform_remote_state.<NAME>.outputs.<ATTRIBUTE> - see lines 32, 33
data "terraform_remote_state" "db" {
    backend = "s3"

    config = {
        bucket  = "leakespeake-terraform-state-files"
        key     = "global/s3/Chapter3/stage/datastores/mysql/terraform.tfstate"
        region  = "us-east-2"
    }
